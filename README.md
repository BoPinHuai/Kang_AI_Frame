...
基于 RAG（检索增强生成）的个人知识库，支持上传 PDF / Word / Markdown 文档，通过向量检索 + 大模型回答问题。
...
> **首次启动**会自动下载 embedding 模型（约 100MB）。国内网络较慢时，打开 `embedder.py` 取消镜像那行的注释。

## 使用说明

- 点击左侧 📂 上传文档，支持多级文件夹
- 提问时输入 `@文件夹名` 或 `@文件名` 可限定检索范围
- 点击 ⚙ 可切换模型，点击调参图标可调整所有参数
